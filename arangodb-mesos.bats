...
setup() {
  ...
  let end=$(date +%s)+100
    while [ "$(curl -D /dev/stderr -s $CURRENT_IP:8080/ping | tee /dev/stderr)" != "pong" ]; do
      sleep 1
      [ "$end" -gt "$(date +%s)" ]
    done
  }
  ...
  @test "Killing a dbserver will automatically restart that task" {
    ...
    let end=$(date +%s)+100
    while [ $(curl -D /dev/stderr -s http://$CURRENT_IP:5050/master/state.json | tee /dev/stderr | jq -r '.frameworks | map(select (.name == "ara")) | .[0].tasks | map(select (.name == "ara-DBServer1" and .state == "TASK_RUNNING")) | length') != 1 ]; do
      [ "$end" -gt "$(date +%s)" ]
    done
  }
  ...
  @test "Killing a coordinator will automatically restart that task" {
    ...
    let end=$(date +%s)+100
    while [ $(curl -D /dev/stderr -s http://$CURRENT_IP:5050/master/state.json | tee /dev/stderr | jq -r '.frameworks | map(select (.name == "ara")) | .[0].tasks | map(select (.name == "ara-Coordinator1" and .state == "TASK_RUNNING")) | length') != 1 ]; do
      sleep 1
      [ "$end" -gt "$(date +%s)" ]
    done
  }
  ...
  @test "A returning coordinator should have the same amount of collections" {
    ...
    local num_collections=$(curl -D /dev/stderr -s $endpoint/_api/collections | tee /dev/stderr | jq length)
    ...
    local endpoint=$(taskname2endpoint ara-Coordinator1)
    local num_collections_new=$(curl -D /dev/stderr -s $endpoint/_api/collections | tee /dev/stderr | jq length)
  ...
  @test "Killing a secondary server will immediately restart that task" {
    ...
    let end=$(date +%s)+100
    while [ $(curl -D /dev/stderr -s http://$CURRENT_IP:5050/master/state.json | tee /dev/stderr | jq -r '.frameworks | map(select (.name == "ara")) | .[0].tasks | map(select (.name == "ara-Secondary1" and .state == "TASK_RUNNING")) | length') != 1 ]; do
      sleep 1
      [ "$end" -gt "$(date +%s)" ]
    done
  }
  ...
  @test "When a machine containing a primary db server is going down there will be a failover to the secondary" {
    ...
    local endpoint=$(taskname2endpoint ara-Coordinator1)
    curl -D /dev/stderr -s -X POST --data-binary @- --dump - "$endpoint"/_api/collection <<EOF | tee /dev/stderr
  { 
    "name" : "clustertest", "numberOfShards": 2, "waitForSync": true
  }
EOF
    curl -D /dev/stderr -s -X POST --data-binary @- --dump - "$endpoint"/_api/document?collection=clustertest <<EOF | tee /dev/stderr
...
EOF
  ...
  { "Du hast gewiss": "Zeit" }
EOF
    local num_docs=$(curl -D /dev/stderr -s "$endpoint"/_api/document?collection=clustertest | tee /dev/stderr | jq '.documents | length')
    ...
    local num_docs_new=$(curl -D /dev/stderr -s "$endpoint"/_api/document?collection=clustertest | tee /dev/stderr | jq '.documents | length')
